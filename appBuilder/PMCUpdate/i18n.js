module.exports = {
    "zh-CN": {
        versionPrefix: "版本",
        whatsNewItems: `
        新增窗口绑定功能，可在识别区域设置全局窗口绑定和单个步骤的窗口绑定，绑定窗口后可限制图像识别范围
        新增图像识别范围设置功能，可在屏幕指定位置设置指定大小的图像识别矩形范围，可全局和单步骤设置
        新增相对坐标转换功能，在绑定窗口后可将目标坐标转换为相对坐标，使用相对坐标后即使窗口位置移动也能点击到正确坐标
    ...
    "zh-TW": {
        versionPrefix: "版本",
        whatsNewItems: `
        新增視窗綁定功能，可在識別區域設置全域視窗綁定和單個步驟的視窗綁定，綁定視窗後可限制圖像識別範圍
        新增影像識別範圍設定功能，可在螢幕指定位置設定指定大小的影像識別矩形範圍，可全域和單步驟設定
        新增相對座標轉換功能，在綁定視窗後可將目標座標轉換為相對座標，使用相對座標後即使視窗位置移動也能點擊到正確座標
    ...
    "en": {
        ...
        whatsNewItems: `
        Added window binding function, which allows global window binding and single step window binding to be set in the recognition area. After binding the window, the image recognition range can be restricted
        ...
        Added relative coordinate conversion functionality. After binding windows, target coordinates can be converted to relative coordinates. Using relative coordinates ensures accurate clicking even if the window position moves
